<?php
	...
	//register
	if (isset($_POST["registrar"])) {//verifica si existe o se presiono registrar
		$nombre=mysqli_real_escape_string($conexion, $_POST['nombre']);
		//valida que no se inyecte SQL
		$correo=mysqli_real_escape_string($conexion, $_POST['correo']);
		$user=mysqli_real_escape_string($conexion, $_POST['user']);
		$pass=mysqli_real_escape_string($conexion, $_POST['pass']);
		$password_encriptada=sha1($pass);
		$sqluser="SELECT idusuarios FROM usuarios WHERE usuario='$user'"; //no deja registrar un usuario ya registrado
		$resultadouser=$conexion->query($sqluser);
		$filas=$resultadouser->num_rows;//cuenta los registros donde el usuario es lo que hay en $user
		if($filas>=1){
			echo "<script>
